...
const backupRemoteFile = async (config, index) => {
  try {
    ...
    // 使用PowerShell的Compress-Archive命令来压缩文件夹
        await new Promise((resolve, reject) => {
			exec(`powershell -Command "Compress-Archive -Path '${windowsPath}\\*' -DestinationPath '${bakDir}\\${zipFileName}' -Force"`, (error, stdout, stderr) => {
        if (error) {
	...
	// 删除windowsPath目录下的所有非ZIP文件和子目录
	await new Promise((resolve, reject) => {
  exec(`powershell -Command "Get-ChildItem -Path '${windowsPath}' -Recurse | ForEach-Object { if ($_.Name -ne 'dist.zip' -and $_.FullName -ne '${windowsPath}\\dist.zip') { Remove-Item -Path $_.FullName -Recurse -Force } }"`, (error, stdout, stderr) => {
    if (error) {
...
const removeRemoteFile = async (config, index) => {
  try {
    ...
    // 删除压缩文件
    await new Promise((resolve, reject) => {
      exec(`powershell -Command "Remove-Item '${remoteFileName}' -Force"`, (error, stdout, stderr) => {
        if (error) {
          console.error('删除失败:', error);
          reject(error);
        } else {
...
const unzipRemoteFile = async (config, index) => {
  ...
  // 使用PowerShell命令进行解压
    await new Promise((resolve, reject) => {
      exec(`powershell -Command "Expand-Archive -Path '${remoteFileName}' -DestinationPath '${windowsPath}' -Force"`, (error, stdout, stderr) => {
        if (error) {
